...
describe("Test messages part of User class", function () {
  ...
  test('can get messages from user', async function () {
    ...
    console.log('Received messages from user:', m);
    expect(m).toEqual([{
      ...
      read_at: null,
      from_username: "test1",
      from_first_name: "Test1",
      from_last_name: "Testy1",
      from_phone: "+14155550000",
      to_username: "test2",  // Add this line to match the received data
    }]);
  });
  
  test('can get messages to user', async function () {
    let m = await User.messagesTo("test1");
    console.log('Received messages to user:', m);
    expect(m).toEqual([{
      id: expect.any(Number),
      body: "u2-to-u1",
      sent_at: expect.any(Date),
      read_at: null,
      from_username: "test2",  // Add this line to match the received data
      to_username: "test1",
      to_first_name: "Test1",
      to_last_name: "Testy1",
